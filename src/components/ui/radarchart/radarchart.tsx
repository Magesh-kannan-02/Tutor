...
const getResponsiveConfig = (width?: number) => {
  const w = width || (typeof window !== "undefined" ? window.innerWidth : 1000)
...
const roundedGridPlugin: Plugin<"radar"> = {
  ...
  beforeDraw(chart: Chart<"radar">) {
    ...
    ticks.forEach((tick, index) => {
      ...
      const cornerRadius = getResponsiveConfig(chart.width).radius // Responsive radius
...
const customLabelsPlugin: Plugin<"radar"> = {
  ...
  afterDraw(chart: Chart<"radar">) {
    ...
    labels.forEach((label, index) => {
      ...
      const config = getResponsiveConfig(chart.width)
